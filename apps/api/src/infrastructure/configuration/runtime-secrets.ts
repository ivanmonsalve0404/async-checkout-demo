...
const parseRuntimeSecret = (source: string, sandboxRequired: boolean): RuntimeSecrets => {
  let value: unknown;
  try {
  ...
  const expectedKeys = sandboxRequired
    ? [
        'integritySecret',
  ...
  if (!sandboxRequired) {
    return { runtimeSecurityRootKey: rootKey, sandbox: undefined };
  }
  const {
    publicKey,
    privateKey,
...
export const loadRuntimeSecrets = async (
  ...
  if (config.runtimeSecretArn === undefined) {
    return { runtimeSecurityRootKey: config.runtimeSecurityRootKey, sandbox: undefined };
  }
  const response = await reader.send(
    new GetSecretValueCommand({ SecretId: config.runtimeSecretArn }),
  );

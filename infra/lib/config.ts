...
export interface ReleaseConfig {
  ...
  readonly publicationMode: 'INITIAL_CLOSED' | 'EPHEMERAL_NON_PUBLIC';
...
export function parseFoundationConfig(raw: RawFoundationConfig): FoundationConfig {
  ...
  if (
    (environment === 'assessment-release' && publicationModeValue !== 'INITIAL_CLOSED') ||
    (environment.startsWith('assessment-prerelease-') &&
      publicationModeValue !== 'EPHEMERAL_NON_PUBLIC')
  ) {
    throw new Error(
      'assessment-release requires INITIAL_CLOSED and prerelease requires EPHEMERAL_NON_PUBLIC',
    );
  }
  ...
  if (paymentAdapter === 'fake') {
    ...
    if (!schedulerEnabled) throw new Error('sandbox release requires the reconciler enabled');
  ...
  if (
    ...
    throw new Error('the initial full release requires sandbox mode and a complete custom domain');
  }

...
export class ReleaseApiStack extends ReleaseStack {
  ...
  public constructor(scope: Construct, id: string, props: ReleaseApiStackProps) {
    ...
    this.releaseOutput(
      'SchedulerStatus',
      config.schedulerEnabled ? publicationStatus : 'DISABLED_EXPLICIT',
  ...
  private grantTableAccess(apiRole: iam.Role, workerRole: iam.Role, data: ReleaseDataStack): void {
    const indexArn = data.checkoutTable.tableArn + '/index/' + RECONCILE_INDEX;
    this.addPolicy(
      apiRole,
      'ApiCatalogReads',
    ...
    this.addPolicy(
      ...
      [data.checkoutTable.tableArn, indexArn],
    );
    this.addPolicy(
      apiRole,
      'ApiTableWrites',

...
function release(overrides: RawFoundationConfig = {}): RawFoundationConfig {
  return {
    ...
    publicationMode: 'VERSIONED_UPDATE_CLOSED',
...
void describe('foundation configuration', () => {
  ...
  void test('keeps a versioned full-release update closed until explicit activation', () => {
    const full = {
    ...
    assert.equal(fullConfig.publicationMode, 'VERSIONED_UPDATE_CLOSED');
    assert.throws(
      () => parseFoundationConfig(release({ ...full, publicationMode: 'EPHEMERAL_NON_PUBLIC' })),
      /requires VERSIONED_UPDATE_CLOSED/,
    );
    assert.throws(
      () =>
        parseFoundationConfig(
      ...
      () =>
        parseFoundationConfig(
          release({
            ...
            runtimeSecretVersionId: undefined,
          }),
        ),
    ...
    assert.throws(
      () =>
        parseFoundationConfig(
          release({
            ...sandbox,
            environment: 'assessment-prerelease-missing-edge-gate',
            publicationMode: 'EPHEMERAL_NON_PUBLIC',
          }),
        ),
      /key-group\/public-key IDs and a runtime secret/,
    );
    assert.throws(
      () =>
        parseFoundationConfig(
  ...
  void test('rejects ambiguous identity, partial domains and cross-region secrets', () => {
    ...
    assert.throws(
      () => parseFoundationConfig(release({ cleanupExpiresAtUtc: undefined })),
      /exact UTC timestamp on expiresOn/,
    );
    assert.throws(
      () => parseFoundationConfig(release({ cleanupExpiresAtUtc: '2099-12-30T23:00:00.000Z' })),
      /exact UTC timestamp on expiresOn/,
    );
    assert.throws(
      () =>

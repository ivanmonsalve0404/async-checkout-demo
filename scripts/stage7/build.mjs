...
export const selfTestReleaseBuild = async () => {
  ...
  const selfTestRoot = path.join(repositoryRoot, '.tmp', 'stage7-release-build-self-test-');
  await mkdir(path.dirname(selfTestRoot), { recursive: true });
  const temporary = await mkdtemp(selfTestRoot);
  try {

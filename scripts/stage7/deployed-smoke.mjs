...
const capabilityCookieContractSha256 = (maxAgeSeconds) =>
  sha256(
    JSON.stringify({
      name: CAPABILITY_COOKIE_NAME,
...
export const validateCapabilityCookieEvidence = (value) => {
  if (
    !exactKeys(value, [
      'schemaVersion',
      'stage',
      ...
      'maxAgeBounded',
      'domainAbsent',
      'attributeContractSha256',
      'containsSensitiveData',
    ]) ||
    value.schemaVersion !== 1 ||
    value.stage !== 7 ||
    value.kind !== 'CHECKOUT_CAPABILITY_COOKIE_OBSERVATION' ||
    value.status !== 'PASS' ||
    !utc(value.observedAtUtc) ||
    value.cookieNameSha256 !== sha256(CAPABILITY_COOKIE_NAME) ||
    value.pathExact !== true ||
    value.httpOnly !== true ||
    value.secure !== true ||
    value.sameSiteStrict !== true ||
    !Number.isSafeInteger(value.maxAgeSeconds) ||
    value.maxAgeSeconds < 60 ||
    value.maxAgeSeconds > CAPABILITY_COOKIE_MAX_AGE_SECONDS ||
    value.maxAgeBounded !== true ||
    value.domainAbsent !== true ||
    value.attributeContractSha256 !== capabilityCookieContractSha256(value.maxAgeSeconds) ||
    value.containsSensitiveData !== false
  ) {
    fail('E7_SMOKE_CAPABILITY_COOKIE_EVIDENCE_INVALID');
  }
  return value;
};

export const captureCapabilityCookieEvidence = (
  headers,
  observedAtUtc = new Date().toISOString(),
) => {
  if (!Array.isArray(headers) || headers.length < 1 || !utc(observedAtUtc)) {
    fail('E7_SMOKE_CAPABILITY_COOKIE_HEADER_INVALID');
  }
  const matches = [];
  for (const header of headers) {
    ...
    if (separator < 1 || parts[0].slice(0, separator) !== CAPABILITY_COOKIE_NAME) continue;
    const cookieValue = parts[0].slice(separator + 1);
    if (
      cookieValue.length < 8 ||
      cookieValue.length > 4096 ||
      [...cookieValue].some((character) => character.codePointAt(0) <= 0x20) ||
    ...
    for (const part of parts.slice(1)) {
      ...
      const name = (attributeSeparator === -1 ? part : part.slice(0, attributeSeparator))
        .trim()
        .toLowerCase();
      const attributeValue =
        attributeSeparator === -1 ? null : part.slice(attributeSeparator + 1).trim();
  ...
  if (matches.length !== 1) fail('E7_SMOKE_CAPABILITY_COOKIE_HEADER_INVALID');
  const attributes = matches[0];
  const maxAge = attributes.get('max-age');
  const maxAgeSeconds = /^[1-9][0-9]{0,5}$/u.test(maxAge ?? '') ? Number(maxAge) : Number.NaN;
  if (
    [...attributes.keys()].toSorted().join('\0') !==
      ['httponly', 'max-age', 'path', 'samesite', 'secure'].toSorted().join('\0') ||
    attributes.get('path') !== CAPABILITY_COOKIE_PATH ||
    attributes.get('httponly') !== null ||
    attributes.get('secure') !== null ||
    attributes.get('samesite') !== 'Strict' ||
    !Number.isSafeInteger(maxAgeSeconds) ||
    maxAgeSeconds < 60 ||
    maxAgeSeconds > CAPABILITY_COOKIE_MAX_AGE_SECONDS ||
    attributes.has('domain')
  ) {
    fail('E7_SMOKE_CAPABILITY_COOKIE_HEADER_INVALID');
  }
  return validateCapabilityCookieEvidence({
    schemaVersion: 1,
    stage: 7,
    kind: 'CHECKOUT_CAPABILITY_COOKIE_OBSERVATION',
    status: 'PASS',
    observedAtUtc,
    cookieNameSha256: sha256(CAPABILITY_COOKIE_NAME),
    pathExact: true,
    httpOnly: true,
    secure: true,
    sameSiteStrict: true,
    ...
    maxAgeBounded: true,
    domainAbsent: true,
    attributeContractSha256: capabilityCookieContractSha256(maxAgeSeconds),
...
/**
 * Produces one real, synthetic pending payment and returns immediately while
 * the scheduler can reconcile it across the N -> N-1 transition. Raw checkout
 * and transaction identifiers never leave the browser process.
 */
export const prepareVersionedRollbackPendingCanary = async ({
  ...
  if (
    ...
    fail('E7_ROLLBACK_PENDING_IDENTITY_INVALID');
  }
  ...
  try {
    ...
    if (signedCookies.length > 0) await context.addCookies(signedCookies);
    ...
    const accepted = await bodyJson(response, 'E7_ROLLBACK_PENDING_RESPONSE_INVALID');
    const progress = await readProgress(page);
    const observed = await pageJson(page, `/api/v1/transactions/${accepted.transactionId}`);
    if (
      response.status() !== 202 ||
      !OPAQUE_ID.test(accepted.transactionId ?? '') ||
      accepted.submissionState !== 'ACCEPTED' ||
      progress.transactionId !== accepted.transactionId ||
      observed.status !== 200 ||
      ...
      fail('E7_ROLLBACK_PENDING_NOT_OBSERVED');
    }
    return {
      schemaVersion: 1,
      stage: 7,
      ...
      requests,
      syntheticOnly: true,
      expectedTerminalStatus: inputs.cards.pending.expectedStatus,
      containsSensitiveData: false,
    };
  } catch (error) {
    if (error instanceof Stage7SmokeError) throw error;
    fail('E7_ROLLBACK_PENDING_BROWSER_FAILURE');
  } finally {
...
export const runCanonicalStage7Smoke = async ({
  ...
  if (
    ...
    (beforeRequest !== undefined && typeof beforeRequest !== 'function')
  ) {
  ...
  try {
    ...
    await context.route('**/*', async (route) => {
      ...
      if (requestOrigin === origin) {
        try {
          beforeRequest?.('ownedTarget');
        } catch (error) {
          requestBudgetError ??= error;
          await route.abort('blockedbyclient');
          return;
        }
        requests.ownedOrigin += 1;
    ...
    const checkoutCreationResponsePromise = page.waitForResponse((response) => {
      const request = response.request();
      const url = new URL(response.url());
      return (
        request.method() === 'POST' && url.origin === origin && url.pathname === '/api/v1/checkouts'
      );
    });
    await page.getByTestId('product-checkout-cta').click();
    const checkoutCreationResponse = await checkoutCreationResponsePromise;
    if (checkoutCreationResponse.status() !== 201) {
      fail('E7_SMOKE_CAPABILITY_COOKIE_RESPONSE_INVALID');
    }
    const capabilityCookie = captureCapabilityCookieEvidence(
      await checkoutCreationResponse.headersArray(),
    );
    ...
    mark('SMK-E7-04', {
      ...
      capabilityCookieSecure: capabilityCookie.status === 'PASS',
    });
    ...
    if (requestBudgetError !== undefined) throw requestBudgetError;
    if (error instanceof Stage7SmokeError) throw error;
...
export const selfTestDeployedSmoke = () => {
  ...
  const capabilityCookieValue = 'checkout-fixture.secret-capability';
  const capabilityCookie = captureCapabilityCookieEvidence(
    [
      { name: 'set-cookie', value: 'unrelated=value; Path=/; Secure' },
      {
        name: 'Set-Cookie',
        value: `${CAPABILITY_COOKIE_NAME}=${capabilityCookieValue}; Path=/api/v1; HttpOnly; Secure; SameSite=Strict; Max-Age=86400`,
      },
    ],
    '2026-08-17T12:00:00.000Z',
  );
  assert.equal(validateCapabilityCookieEvidence(capabilityCookie), capabilityCookie);
  assert.equal(capabilityCookie.maxAgeSeconds, 86_400);
  assert.equal(JSON.stringify(capabilityCookie).includes(capabilityCookieValue), false);
  for (const setCookie of [
    `${CAPABILITY_COOKIE_NAME}=fixture.value; Path=/api/v1; Secure; SameSite=Strict; Max-Age=86400`,
    `${CAPABILITY_COOKIE_NAME}=fixture.value; Path=/api/v1; HttpOnly; Secure; SameSite=Lax; Max-Age=86400`,
    `${CAPABILITY_COOKIE_NAME}=fixture.value; Path=/api/v1; HttpOnly; Secure; SameSite=Strict; Max-Age=86401`,
    `${CAPABILITY_COOKIE_NAME}=fixture.value; Path=/api/v1; HttpOnly; Secure; SameSite=Strict; Max-Age=86400; Domain=example.test`,
  ]) {
    assert.throws(
      () =>
        captureCapabilityCookieEvidence(
          [{ name: 'set-cookie', value: setCookie }],
          '2026-08-17T12:00:00.000Z',
        ),
      (error) => error.code === 'E7_SMOKE_CAPABILITY_COOKIE_HEADER_INVALID',
    );
  }
  assert.throws(
    () =>
      captureCapabilityCookieEvidence(
        [
          {
            name: 'set-cookie',
            value: `${CAPABILITY_COOKIE_NAME}=fixture.one; Path=/api/v1; HttpOnly; Secure; SameSite=Strict; Max-Age=86400`,
          },
          {
            name: 'set-cookie',
            value: `${CAPABILITY_COOKIE_NAME}=fixture.two; Path=/api/v1; HttpOnly; Secure; SameSite=Strict; Max-Age=86400`,
          },
        ],
        '2026-08-17T12:00:00.000Z',
      ),
  ...
  assert.throws(
    () =>
      validateCapabilityCookieEvidence({ ...capabilityCookie, rawValue: capabilityCookieValue }),
    (error) => error.code === 'E7_SMOKE_CAPABILITY_COOKIE_EVIDENCE_INVALID',
  );
  for (const mutate of [
    (entry) => {

...
const runBrowserFocal = async ({ browserType, engine, origin, axe, signedCookies }) => {
...
const runLighthouse = async ({ origin, workspaceRoot, executablePath, signedCookies }) => {
  ...
  for (let index = 0; index < 3; index += 1) {
    ...
    try {
      ...
      if (signedCookies.length > 0) await page.setCookie(...signedCookies);
      ...
      const result = await runtime.lighthouse.navigation(page, `${origin}${PRODUCT_PATH}`, {
        flags: {
          ...
          disableStorageReset: signedCookies.length > 0,
        },
      });
